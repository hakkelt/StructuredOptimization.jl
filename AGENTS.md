...
**Dependencies**: `AbstractOperators.jl`, `ProximalOperators.jl`, `ProximalAlgorithms.jl` are dev'd locally via `[sources]` in the root `Project.toml`, `test/Project.toml`, `benchmark/Project.toml` and `docs/Project.toml` (all four must stay byte-identical for these three), pointing at `/project/c_mrrecon/MriReconstructionToolbox/.vendor-work/{AbstractOperators,ProximalOperators,ProximalAlgorithms}`. Those are **not** plain sibling checkouts to edit directly — they are git worktrees on an `integration` branch that MriReconstructionToolbox's `tools/vendor.jl rebuild` (re)builds from a manifest, `deps/vendor.toml`: reset to `base`, then merge every branch that manifest lists for that package, in order. A `rebuild` run **overwrites** any direct edit to a `.vendor-work/` checkout, so:

- A real bug found in one of these three packages gets fixed on the manifest branch that owns the affected code (find it with `git -C <fork checkout> log -S'<symbol>' --oneline -- <file>`, or match the commit message against `deps/vendor.toml`'s stack entries), pushed there, then picked up by re-running `julia tools/vendor.jl rebuild <Package>` from `MriReconstructionToolbox/`.
- `rebuild` will refuse with a `git worktree add --force` conflict if some other worktree already holds that package's `integration` branch checkout (stray worktrees accumulate under `.claude/worktrees/**/.vendor-work/` from other sessions) — remove the stray with `git worktree remove --force <path>` first, from the fork's own checkout (e.g. `/project/c_mrrecon/AbstractOperators`).
- `ProximalCore.jl` has no `integration` branch or manifest entry; it stays a plain sibling checkout at `../ProximalCore.jl` (`../../ProximalCore.jl` from `test/`, `benchmark/`, `docs/`), currently on `master`.
- After any rebuild, re-`Pkg.resolve()` in a **fresh** Julia session — a session that already loaded the old checkout at the same `.vendor-work/` path will not pick up the new commit, `using` is a no-op for an already-loaded module regardless of what changed on disk.
