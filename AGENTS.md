...
**Dependencies**: `AbstractOperators.jl`, `ProximalOperators.jl`, `ProximalAlgorithms.jl`, `ProximalCore.jl` need local, machine-specific `[sources]` entries (all four `Project.toml` — root, `test/`, `benchmark/`, `docs/` — must stay byte-identical for these) to resolve at all, but those absolute local paths must **never** be pushed. They are kept local-only via `git update-index --skip-worktree` on all four files: git treats them as unchanged regardless of local edits, so the committed content (no `[sources]`) is never touched by an ordinary commit. `Pkg.develop(path = "...")` rewrites `[sources]` (Julia 1.11+ has no `[sources]`-only-in-Manifest mode), so it's safe to run against a skip-worktree'd file. To change what's actually committed, `git update-index --no-skip-worktree <file>` first. Ask the user for the current local checkout paths — where and how those dependencies are vendored/managed is user- and machine-specific, not part of this repo.
