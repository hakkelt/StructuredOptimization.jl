...
~x .= 0.0
~y .= 0.0
slv, = @minimize ls(A * x - B * y + b) st norm(x, 2) <= 1.0e4, norm(y, 2) <= 1.0 with PANOCplus()
~x .= 0.0
slv, = @minimize ls(A * x - b) st norm(x, 2) <= 1.0 with PANOCplus()
~x .= 0.0
slv, = @minimize ls(A * x - b) st norm(x, 2) <= 1.0
~x .= 0.0
...
prob_lasso = problem(ls(A * x - b) + 1.0e-3 * norm(x, 1))
...
let A_ms = randn(5, 10), b_ms = randn(5)
    x_ms = Variable(10)
    sol_ms = solve(problem(ls(A_ms * x_ms - b_ms) + 1.0e-3 * norm(x_ms, 1)), (PANOCplus(maxit = 20), ZeroFPR(maxit = 20)))
...
slv, = @minimize ls(sigmoid(A * x, 10) - b) + norm(x, 1) with PANOCplus(tol = 1.0e-6)
xpg = copy(~x)
~x .= 0.0
slv, = @minimize ls(sigmoid(A * x, 10) - b) + norm(x, 1) with ZeroFPR(tol = 1.0e-6)
xzfpr = copy(~x)
~x .= 0.0

@test norm(xzfpr - xpg) <= 1.0e-4
...
function test_solver(solver)
    ...
    @minimize cf + 1.0e-10 * norm(x, 1) + 1.0e-10 * norm(y, 1) with solver

    @test norm(~x - [a]) < 1.0e-4
    return @test norm(~y - [a^2]) < 1.0e-4
end
solvers = [ZeroFPR(; tol = 1.0e-6), PANOCplus(; tol = 1.0e-6)]
...
let A = randn(5, 4), b = randn(5)
    ...
    ~x .= 0.0
    sol = solve(prob, (PANOCplus(tol = 1.0e-6),))
    @test !isnothing(sol)
...
let A = randn(5, 4), b = randn(5)
    x = Variable(4)
    ~x .= 0.0
    @minimize ls(A * x - b) st norm(x, 2) <= 1.0 with PANOCplus(tol = 1.0e-6)
