...
x_ridge = (A'A + 2.0e-3 * I) \ (A'b)  # minimizer of ½‖Ax - b‖² + 1e-3‖x‖²

x_cg_exact = Variable(5)
solve(problem(ls(A * x_cg_exact - b) + 1.0e-3 * norm(x_cg_exact, 2)^2), CGNR(maxit = 200))
@test ~x_cg_exact ≈ x_ridge rtol = 1.0e-4
...
x_ref_l1 = Variable(4)
solve(problem(ls(A_tall * x_ref_l1 - b_tall) + 1.0e-2 * norm(x_ref_l1, 1)), PANOCplus(maxit = 2000))

x_admm_l1 = Variable(4)
solve(problem(ls(A_tall * x_admm_l1 - b_tall) + 1.0e-2 * norm(x_admm_l1, 1)), ADMM(maxit = 5000, rho = 1.0))
@test ~x_admm_l1 ≈ ~x_ref_l1 rtol = 1.0e-4

# `ls`'s normal-op formulation hands its already-built `AᴴA` to ADMM instead of letting ADMM
# build a second one (`prepare` emits the assumption's `AHA` key). The cached operator must
