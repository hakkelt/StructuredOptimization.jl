...
@test norm(affine(ex) * (~variables(ex)) - (A1 * (~x1)) * (A2 * (~x2))) < 1.0e-12
ex = (opA1 * x1 - b1) * (opA2 * x2)
@test variables(ex) == (x1, x2)
@test norm(affine(ex) * (~variables(ex)) - (A1 * (~x1) - b1) * (A2 * (~x2))) < 1.0e-12
ex = (opA1 * x1) * (opA2 * x2 + b2)
@test variables(ex) == (x1, x2)
@test norm(affine(ex) * (~variables(ex)) - (A1 * (~x1)) * (A2 * (~x2) + b2)) < 1.0e-12
ex = (opA1 * x1 + b1) * (opA2 * x2 + b2)
@test variables(ex) == (x1, x2)
@test norm(affine(ex) * (~variables(ex)) - (A1 * (~x1) + b1) * (A2 * (~x2) + b2)) < 1.0e-12
ex = (opA1 * x1 - b1) * (opA1 * x1 + b1)
@test variables(ex) == (x1,)
@test norm(affine(ex) * (~variables(ex)) - (A1 * (~x1) - b1) * (A1 * (~x1) + b1)) < 1.0e-12
...
ex = reshape(x, 4, 2)
@test norm(operator(ex) * ~x - reshape(~x, 4, 2)) < 1.0e-12

ex2 = reshape(A * x, 2, 5)
@test norm(operator(ex2) * ~x - reshape(A * ~x, 2, 5)) < 1.0e-12

ex3 = reshape(A * x, 2, 5) + B
@test norm(operator(ex2) * ~x + displacement(ex3) - reshape(A * ~x, 2, 5) - B) < 1.0e-12

ex4 = reshape(A * x - b, 2, 5)
@test norm(operator(ex4) * ~x + displacement(ex4) - reshape(A * ~x - b, 2, 5)) < 1.0e-12
...
@test norm(operator(ex1) * ~x - (opA + opB) * ~x) < 1.0e-12
...
@test norm(operator(ex1) * (~variables(ex1)) - hcat(opA, opB) * (~variables(ex1))) < 1.0e-12
...
@test norm(operator(exs1) * (~variables(exs1)) - hcat(opA + opA2, opB) * (~variables(exs1))) < 1.0e-12
@test variables(exs2) == (xa, xb)
@test norm(operator(exs2) * (~variables(exs2)) - hcat(opA + opA2, opB) * (~variables(exs2))) < 1.0e-12

# (+) sum of different variables with HCAT
exs3 = exs1 + exs2
@test variables(exs3) == (xa, xb)
@test norm(operator(exs3) * (~variables(exs3)) - hcat(2 * (opA + opA2), 2 * opB) * (~variables(exs3))) < 1.0e-12
# (+) sum of different variables with HCAT
exs4 = exs1 + (xc + xd)
@test variables(exs4) == (xa, xb, xc, xd)
@test norm(operator(exs4) * (~variables(exs4)) - hcat(opA + opA2, opB, opI, opI) * (~variables(exs4))) < 1.0e-12

# (+) sum Array
ex1 = xd + b
@test norm(displacement(ex1) - b) == 0.0

# (+) sum scalar
ex2 = opB * xb + b0
@test (displacement(ex2) - b0) == 0.0

# (+) sum displacemented expressions
ex3 = ex1 + ex2
@test norm(displacement(ex3) - (b .+ b0)) == 0.0
...
x2 = Variable(randn(n))
ex1 = x1 .+ x2
@test norm(operator(ex1) * (~variables(ex1)) - ((~x1) .+ (~x2))) < 1.0e-9
...
@test begin
    ...
    norm(output - expected_output) < 1.0e-12
end
...
let
    ...
    @test norm(out1 - expected1) < 1.0e-12
    ...
    @test norm(out2 - expected2) < 1.0e-12
    ...
    @test norm(out3 - expected3) < 1.0e-12
end
