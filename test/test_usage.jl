...
@test maximum(subdiff1_low + grad1) <= 1.0e-6
@test maximum(-subdiff1_upp - grad1) <= 1.0e-6
@test norm(grad2 + subgr2) <= 1.0e-6
...
@test norm(~x1_panocplus - ~x1_ffb, Inf) / (1 + norm(~x1_ffb, Inf)) <= 1.0e-6
@test norm(~x2_panocplus - ~x2_ffb, Inf) / (1 + norm(~x2_ffb, Inf)) <= 1.0e-6
...
b = A * x_star + A' \ y_star
@test norm(A' * (A * x_star - b) + lam * sign.(x_star)) <= 1.0e-12
...
prob = problem(expr)
@time sol = solve(prob, PANOCplus(tol = 1.0e-10, verbose = false))

@test norm(~x_pg - x_star, Inf) <= 1.0e-8
@test norm(A' * (A * ~x_pg - b) + lam * sign.(~x_pg)) <= 1.0e-6
...
@test norm(~x_fpg - x_star, Inf) <= 1.0e-8
@test norm(A' * (A * ~x_fpg - b) + lam * sign.(~x_fpg)) <= 1.0e-6
...
let A_tiny = randn(5, 3), b_tiny = randn(5), lam_tiny = 0.1
    ...
    prob = problem(expr)
    @time sol = solve(prob, ZeroFPR(tol = 1.0e-4, verbose = false))
...
prob = problem(expr)
@time sol = solve(prob, FastForwardBackward(tol = 1.0e-10, verbose = false))

@test norm(~x_ffb - x_star, Inf) <= 1.0e-6
@test norm(A' * (A * ~x_ffb - b) + lam * sign.(~x_ffb)) <= 1.0e-4
...
prob = problem(expr)
@time sol = solve(prob, PANOCplus(tol = 1.0e-6, verbose = false))
...
prob = problem(expr)
@time sol = solve(prob, FastForwardBackward(tol = 1.0e-6, verbose = false))
...
@test norm(~x_pg - ~x_fpg, Inf) / (1 + norm(~x_pg, Inf)) <= 1.0e-2
@test norm(~x_pg - ~x_panoc, Inf) / (1 + norm(~x_pg, Inf)) <= 1.0e-2
...
@test norm(~x_pg - max.(lb, min.(ub, ~x_pg)), Inf) <= 1.0e-12
@test norm(~x_pg - max.(lb, min.(ub, ~x_pg - A' * (A * ~x_pg - b))), Inf) / (1 + norm(~x_pg, Inf)) <= 1.0e-6
...
@test norm(~x_fpg - max.(lb, min.(ub, ~x_fpg)), Inf) <= 1.0e-12
@test norm(~x_fpg - max.(lb, min.(ub, ~x_fpg - A' * (A * ~x_fpg - b))), Inf) / (1 + norm(~x_fpg, Inf)) <= 1.0e-6
...
let A_t = randn(8, 4), b_t = randn(8), lb_t = -1.0, ub_t = 1.0
    ...
    @time sol = solve(prob, ZeroFPR(tol = 1.0e-4, verbose = false))
    @test norm(~x_zerofpr - max.(lb_t, min.(ub_t, ~x_zerofpr)), Inf) <= 1.0e-12
    @test !any(isnan.(~x_zerofpr))
end
...
@test norm(~x_panoc - max.(lb, min.(ub, ~x_panoc)), Inf) <= 1.0e-12
@test norm(~x_panoc - max.(lb, min.(ub, ~x_panoc - A' * (A * ~x_panoc - b))), Inf) / (1 + norm(~x_panoc, Inf)) <= 1.0e-4
...
@time sol = solve(prob, PANOCplus(tol = 1.0e-8, verbose = false))

@test all(~x_pg .>= 0.0)
@test norm(~x_pg - x_star, Inf) / (1 + norm(x_star, Inf)) <= 1.0e-8
...
@test all(~x_fpg .>= 0.0)
@test norm(~x_fpg - x_star, Inf) / (1 + norm(x_star, Inf)) <= 1.0e-8
...
@time sol = solve(prob, FastForwardBackward(tol = 1.0e-8, verbose = false))

@test all(~x_panoc .>= 0.0)
@test norm(~x_panoc - x_star, Inf) / (1 + norm(x_star, Inf)) <= 1.0e-6
...
@test all(~x .>= 0.0)
@test norm(~x - x_star, Inf) / (1 + norm(x_star, Inf)) <= 1.0e-6
